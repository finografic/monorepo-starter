...
export function GitHubProfileCard({
  ...
  if (loading) {
    return (
      <div
        ...
        className={cn('flex flex-col gap-4 p-6 border border-border rounded-lg bg-card', className)}
        {...props}
      >
  ...
  const blogUrl = data.blog ? (data.blog.startsWith('http') ? data.blog : `https://${data.blog}`) : null;
    ...
    >
      ...
      {showStats && (
        <div className="flex gap-4 pt-2 border-t border-border text-sm">
          <span className="flex items-center gap-1.5">
            <span className="font-semibold text-foreground">{formatNumber(data.public_repos)}</span>
            <span className="text-muted-foreground">repos</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="font-semibold text-foreground">{formatNumber(data.followers)}</span>
            <span className="text-muted-foreground">followers</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="font-semibold text-foreground">{formatNumber(data.following)}</span>
